    ...
    const content = `
      <form>
        <div class="form-group">
          <label>Roll Initiative Using:</label>
...
function getInitiativeModifier(actor, rollConfig) {
  ...
  if (type === 'attribute') {
    ...
    const skillLabel = game.i18n.localize(CONFIG.ZWOLF.skills[key]);
    return { 
      modifier, 
...
function getDefaultInitiativeModifier(actor) {
  ...
  const modifier = agility ? calculateProgressionBonus(actor, agility.progression) : 0;
...
function generateInitiativeFlavor(netBoosts, name, attributeOrSkill) {
  ...
  return `Initiative (${attributeOrSkill}) - ${boostText} - ${name}`;
}
